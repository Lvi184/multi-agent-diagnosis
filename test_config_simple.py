...
def test_deepseek_connection():
    ...
    if not settings.deepseek_api_key:
        print("警告: 未配置 DEEPSEEK_API_KEY，将使用规则回退模式")
    ...
    try:
        ...
        print(f"成功: DeepSeek API 连接成功！")
        print(f"   响应: {response}")
        return True
    except Exception as e:
        print(f"错误: DeepSeek API 连接失败: {e}")
        return False
...
def test_agent_system():
    ...
    try:
        ...
        for trace in result.get('agent_traces', []):
            ...
            print(f"完成 {agent_name}: {summary}")

        return True

    except Exception as e:
        print(f"错误: 多 Agent 系统测试失败: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    print("测试总结")
    print("=" * 60)
    print(f"配置加载: {'成功' if has_api_key else '无API Key(规则模式)'}")
    print(f"DeepSeek API: {'正常' if api_ok else '未连接(规则模式)'}")
